...
class View(tk.Tk):
    def __init__(self):
        ...
        self.balance_label = ttk.Label(self, text="Balance:")
        ...
        self.bet.set(1)
        self.bet_spinbox = ttk.Spinbox(self, from_=1, to=1000, textvariable=self.bet)
        ...
        self.number_slider = ttk.Scale(self, from_=5, to=96, orient=tk.HORIZONTAL,
                                       command=lambda x: self.number.set(round(float(x))), variable=self.number)
        ...
        self.wins_label = ttk.Label(self, text="Wins:")
        self.wins_label.grid(row=11, column=0)
        self.wins = tk.IntVar()
        self.wins.set(0)
        self.wins_display = ttk.Label(self, textvariable=self.wins)
        self.wins_display.grid(row=11, column=1)

        self.losses_label = ttk.Label(self, text="Losses:")
        self.losses_label.grid(row=12, column=0)
        self.losses = tk.IntVar()
        self.losses.set(0)
        self.losses_display = ttk.Label(self, textvariable=self.losses)
        self.losses_display.grid(row=12, column=1)

        self.reset_button = ttk.Button(self, text="Reset")
        self.reset_button.grid(row=13, column=0, columnspan=2)
